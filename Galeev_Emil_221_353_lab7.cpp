...
bool modeQuicksort—omparison(human& a, human& b, string mode = "") {
    if (mode == "surname") {
...
template <typename T>
bool modeQuicksort—omparison(T a, T b, string mode = "") {
    if (mode != "") {
        return false;
    }
    return a < b;
...
void quicksort(T* arr, int maxi, string mode = "") {
    ...
    while (last >= first) {
        while (modeQuicksort—omparison(arr[first], mid, mode)) {
            first++;
        }

        while (modeQuicksort—omparison(arr[last], mid, mode)) {
            last--;
        }
